import Link from 'next/link';
import { promises as fs } from 'fs';
import path from 'path';
import FlexExportClient from './FlexExportClient';
import { EXPORT_PRESETS } from './export-presets';
import { parseCsvProducts } from '@/lib/csv-product';
import type { CsvProduct } from '@/lib/csv-product';
import { buildPreviewDocumentFromCsv } from './export-helpers';
import type { ExportTemplateKey } from './export-types';

function parseSkus(raw?: string): string[] {
  if (!raw) return [];
  return raw
    .split(',')
    .map((s) => s.trim())
    .filter(Boolean);
}

export default async function FlexExportPage({
  searchParams,
}: {
  searchParams: Promise<{ skus?: string; template?: string }>;
}) {
  const resolved = await searchParams;
  const requestedSkus = parseSkus(resolved.skus);
  const template = ((resolved.template as ExportTemplateKey) || 'promotion');
  const config = EXPORT_PRESETS[template] || EXPORT_PRESETS.promotion;

  const csvPath = path.join(process.cwd(), 'public', 'data', 'cnypharmacyz.csv');
  let allProducts: CsvProduct[] = [];
  try {
    const text = await fs.readFile(csvPath, 'utf-8');
    allProducts = parseCsvProducts(text);
  } catch {
    allProducts = [];
  }

  const selectedProducts = requestedSkus.length
    ? allProducts.filter((p) => requestedSkus.includes(p.sku))
    : [];

  const preview = buildPreviewDocumentFromCsv(selectedProducts, config);

  return (
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-violet-600">Flex Export Wizard</p>
            <h1 className="text-2xl font-bold text-slate-900">เตรียมส่งออก LINE Flex</h1>
            <p className="mt-1 text-sm text-slate-500">
              ข้อมูลจาก cnypharmacyz.csv — เลือก {selectedProducts.length} รายการ
            </p>
          </div>
          <Link
            href="/"
            className="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100"
          >
            ← กลับหน้าหลัก
          </Link>
        </div>

        {requestedSkus.length === 0 ? (
          <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-900">
            <h2 className="text-lg font-semibold">ยังไม่มีสินค้าที่เลือกสำหรับส่งออก</h2>
            <p className="mt-2 text-sm">
              กรุณากลับไปที่หน้าหลัก เลือกสินค้า แล้วค่อยกดสร้าง Flex Message อีกครั้ง
            </p>
          </section>
        ) : (
          <FlexExportClient initialConfig={config} initialPreview={preview} />
        )}
      </div>
    </main>
  );
}
